...
.tablenav .tablenav-pages a,.tablenav-pages-navspan{display:inline-block;min-width:17px;border:1px solid #ccc;padding:3px 5px 7px;background:#e5e5e5;font-size:16px;line-height:1;font-weight:400;text-align:center}
  ...
  <div class="vx_wrap"> 
  <div>
  <h2  class="vx_img_head"><img alt="<?php esc_html_e("Salesforce Feeds", 'cf7-salesforce') ?>" title="<?php esc_html_e("Salesforce Feeds", 'cf7-salesforce') ?>" src="<?php echo $this->get_base_url()?>images/salesforce-crm-logo.png?ver=1" /> <?php esc_html_e("Salesforce Log", 'cf7-salesforce'); ?>   <select style="min-width: 120px; max-width: 200px; margin-left: 12px; font-weight: normal;" id="vx_sel_feed">
  <option value=""><?php esc_html_e('All Feeds','cf7-salesforce') ?></option>
  <?php   
...
<form id="vx_form" class="crm_form" method="get"><div>
  ...
  <input type="text" placeholder="<?php esc_html_e('Search','cf7-salesforce') ?>" value="<?php echo esc_attr($this->post('search')) ?>" name="search" class="crm_input_inline">
  <?php
...
?>
  ...
  <option value=""><?php esc_html_e('All Objects','cf7-salesforce') ?></option>
  <?php    
  ...
  <option value=""><?php esc_html_e('All Status','cf7-salesforce') ?></option>
  <?php
  ...
  <option value=""><?php esc_html_e('All Times','cf7-salesforce') ?></option>
  <?php
  ...
  <input type="text" name="start_date" placeholder="<?php esc_html_e('From Date','cf7-salesforce') ?>" value="<?php if(isset($_REQUEST['start_date'])){echo esc_attr($_REQUEST['start_date']);}?>" class="vxc_date crm_input_inline" style="width: 100px">
  ...
  <button type="submit" class="button-secondary button crm_input_inline" title="<?php esc_html_e('Apply','cf7-salesforce') ?>" id="vx_apply_bulk"><i class="fa fa-check"></i> <?php esc_html_e('Apply','cf7-salesforce') ?></button>
  ...
  ?>
  <button type="button" name="tab_action" title="<?php esc_html_e('Export as CSV','cf7-salesforce') ?>" id="vx_export" class="button-secondary button crm_input_inline vx_left_10"><i class="fa fa-download"></i> <?php esc_html_e('Export as CSV','cf7-salesforce') ?></button> 
  <?php
  }
  ...
  if(isset($data['feeds'][0]['entry_id']) && $data['feeds'][0]['entry_id']!=""){
     $entry_id=$data['feeds'][0]['entry_id'];
      ?>
  <a href="<?php echo  esc_url( $log_link.'&entry_id='.$entry_id);?>" title="<?php echo sprintf(esc_html__('View Entry# %s Logs','cf7-salesforce'),$entry_id); ?>" class="button vx_left_10"><i class="fa fa-hand-o-right"></i> <?php echo sprintf(esc_html__('View Entry# %s Logs','cf7-salesforce'),$entry_id); ?></a><?php
  }}
 if($this->post('entry_id') !="" || $_log_id !=""){
          ?><a href="<?php echo esc_url($log_link);?>" title="<?php esc_html_e('View All Logs','cf7-salesforce') ?>" class="button vx_left_10"><i class="fa fa-external-link"></i> <?php esc_html_e('View All Logs','cf7-salesforce') ?></a>        
          <?php
      }
...
if($items>0){
  ?>
  <div class="tablenav-pages"> <span id="paging_header" class="displaying-num"><?php esc_html_e('Displaying','cf7-salesforce') ?> <span id="paging_range_min_header"><?php echo esc_html($data['min']); ?></span> - <span id="paging_range_max_header"><?php echo esc_html($data['max']) ?></span> of <span id="paging_total_header"><?php echo esc_html($data['items']) ?></span></span><?php echo wp_kses_post($data['links']) ?></div>
 <?php
}
        ?>       
  </div>
  ...
  <th scope="col" class="manage-column vx_sort"  data-name="crm_id"><?php esc_html_e("Salesforce ID", 'cf7-salesforce') ?>
  <i class="fa fa-caret-<?php echo esc_attr($crm_order) ?> vx_sort_icon <?php echo esc_attr($crm_class) ?>"></i>                          
  </th>
  <th scope="col" class="manage-column vx_sort"  data-name="entry_id"><?php esc_html_e("Entry ID", 'cf7-salesforce') ?>
  <i class="fa fa-caret-<?php echo esc_attr($entry_order) ?> vx_sort_icon <?php echo esc_attr($entry_class) ?>"></i>                   
  </th>
     <th scope="col"><?php esc_html_e('Feed ID', 'cf7-salesforce') ?> </th> 
  <th scope="col" class="manage-column"  data-name="object"><?php esc_html_e("Description", 'cf7-salesforce') ?>
  </th>
  <th scope="col" class="manage-column vx_sort"  data-name="time"><?php esc_html_e("Time", 'cf7-salesforce') ?>
  <i class="fa fa-caret-<?php echo esc_attr($time_order) ?> vx_sort_icon <?php echo esc_attr($time_class) ?>"></i>
  </th>
  <th style="width: 40px"><?php esc_html_e('Detail','cf7-salesforce') ?></th>
  </tr>
  </thead>
  ...
  <th><?php esc_html_e('Detail','cf7-salesforce') ?></th>  
  </tr>
  ...
  <td class="vx_check_col"><input type="checkbox" name="log_id[]" value="<?php echo esc_attr($row['id']); ?>" class="crm_input_check"></td>
    <td class="vx_icon_col"><img src="<?php echo esc_url($base_url) ?>images/<?php echo esc_attr($row["status_img"]); ?>.png" alt="<?php echo $row["status"] ? esc_html__("Active", 'cf7-salesforce') : esc_html__("Inactive", 'cf7-salesforce');?>" title="<?php echo esc_html($row['title']);?>" class="crm_status_img" /></td>
...
$feed_link=$this->get_feed_link($row['feed_id'],$row['form_id']);
 ?><a href="<?php echo esc_url($feed_link) ?>" title="<?php esc_html_e('Edit Feed','cf7-salesforce') ?>">#<?php echo esc_html($row['feed_id']) ?></a><?php  } ?>
 </td>
    ...
    <td><i class="vx_icons vx_detail fa fa-th-list" title="<?php esc_html_e('Expand Details','cf7-salesforce') ?>"></i></td>  
  </tr>
  ...
  <tr>
    <td colspan="4" style="padding:20px;">
        <?php esc_html_e("No Record(s) Found", 'cf7-salesforce'); ?>
    </td>
  </tr>
  <?php
  }
  ...
  ?>
    <div class="crm_actions tablenav">
   <a id="vx_clear_logs" class="button" title="<?php esc_html_e('Clear Salesforce Log','cf7-salesforce') ?>" href="<?php echo esc_url(wp_nonce_url(admin_url('admin.php?page='.$this->post('page')."&view=log&vx_tab_action_".$this->id."=clear_logs"),'vx_nonce','vx_nonce')); ?>"><?php esc_html_e('Clear Salesforce Log','cf7-salesforce') ?></a>
  <div class="tablenav-pages"> <span id="paging_header" class="displaying-num"><?php esc_html_e('Displaying','cf7-salesforce') ?> <span id="paging_range_min_header"><?php echo esc_html($data['min']) ?></span> - <span id="paging_range_max_header"><?php echo esc_html($data['max']) ?></span> of <span id="paging_total_header"><?php echo esc_html($data['items']) ?></span></span><?php echo wp_kses_post($data['links']); ?></div>
    </div>
  <?php
  }
...
if(sel.val() == ""){
    alert('<?php esc_html_e('Please Select Action','cf7-salesforce') ?>');
      return false;
}
if($(".crm_input_check:checked").length == 0){ 
    alert('<?php esc_html_e('Please select at least one entry','cf7-salesforce') ?>');
    return false;
}
var action=sel.val();
 ...
 if(!confirm('<?php esc_html_e('Exporting more than 4 entries may take too long.\\n Are you sure you want to continue?','cf7-salesforce') ?>')){
  e.preventDefault();    
   ...
   $("#vx_clear_logs").click(function(e){
      if(!confirm('<?php esc_html_e('Salesforce Logs will be deleted permanently. Do you want to continue?','cf7-salesforce') ?>')){
          e.preventDefault();
      }  
  })    
  ...
  $(document).on("click",".vx_detail,.vx_close_detail",function(e){
    ...
    if(td.is(":visible"))
    {
      next_tr.find('.vxa_entry').slideUp('fast',function(){
       ...
       if(is_main){
    icon.attr('title','<?php esc_html_e('Expand Detail','cf7-salesforce'); ?>');    
    }        
     return;   
    }else{
     next_tr.show('fast'); 
    ...
    if(is_main){
    icon.attr('title','<?php esc_html_e('Collapse Detail','cf7-salesforce'); ?>');    
    }    
    }
